    ...
    <div class="table-responsive">
        <table class="display" id="myTable">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                    <img class="img-size" src="{{ (isset($item->company_image)) ? asset('storage/company_images/' . $item->company_image) : 'assets/images/nav-user.svg'}}" alt="image">
                    </td>
                    ...
                    <td>
                        <button data-target="#editcompany{{$item->company_id}}" data-toggle="modal" class="btn p-0" style="background: none;">
                        ...
                        <a href="{{ route('company.delete', ['id' => $item->company_id]) }}">
                ...
                <!-- Modal -->
                <div class="modal fade" id="editcompany{{$item->company_id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content" style="border-radius: 15px;">
                            ...
                            <div class="modal-body">
                                <form action="{{ url('company/update/'.$item->company_id) }}" method="POST" id="" enctype="multipart/form-data">
                                    @csrf
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-4">
                                            ...
                                            {{-- <img src="{{asset('storage/staff_images/' . $item->company_image)}}" alt="image">
                                            @endif --}}
                                        </div>
                                        <div class="col-12 mb-3">
                ...
                <script>
                        ...
                        const fileInput{{$item->company_id}} = document.getElementById('fileInput{{$item->company_id}}');
                        const profileImage{{$item->company_id}} = document.getElementById('profileImage{{$item->company_id}}');
                        const form{{$item->company_id}} = document.getElementById('myForm{{$item->company_id}}');

                      
                        // Handle file input change
                        fileInput{{$item->company_id}}.addEventListener('change', function(event) {
                          ...
                          if (file.type.startsWith('image/')) {
                            if (file.size <= 1048576) {
                              ...
                              reader.onload = function(e) {
                                profileImage{{$item->company_id}}.src = e.target.result;
                                form.action = "";
                              };
                            } else {
                              ...
                              fileInput{{$item->company_id}}.value = "";
                            }
                          } else {
